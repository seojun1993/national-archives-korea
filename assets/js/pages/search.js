...
// 기관코드검색 모달 열기
function openSearchOrganList() {
    document.body.style.overflow = 'hidden';
    const modal = document.querySelector('.organ_search_modal');
    if (modal) {
        modal.style.display = 'block';
        modal.setAttribute('aria-hidden', 'false'); // 접근성 보완
    }
...
// 기존 전체 체크박스 기능 (기존 코드와 충돌 방지)
document.addEventListener('DOMContentLoaded', function() {
    const existingAllCheckbox = document.getElementById('styleCd00');
    const existingIndividualCheckboxes = document.querySelectorAll('input[name="styleCdArr"]');
    
    if (existingAllCheckbox) {
        existingAllCheckbox.addEventListener('change', function() {
            const isChecked = this.checked;
            
            existingIndividualCheckboxes.forEach(checkbox => {
                checkbox.checked = isChecked;
            });
        });
        ...
        existingIndividualCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                const allChecked = Array.from(existingIndividualCheckboxes).every(cb => cb.checked);
                
                if (allChecked) {
                    existingAllCheckbox.checked = true;
                } else {
                    existingAllCheckbox.checked = false;
                }
            });
        });
    }
